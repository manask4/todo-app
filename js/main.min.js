...
delete n[s][a],$.isEmptyObject(n[s])?(delete n[s],$(t).parent().remove()):$(e).closest(".notes-list").remove(),localStorage.setItem("notes",JSON.stringify(n))}function get_random_id(){return(Math.random()+1).toString(16).substring(2,7)}var notesStorage=localStorage.getItem("notes")
...
delete_note(t,e)}),$(".user-notes").on("click",".check-icon",function(){$(this).hasClass("fa-check-circle-o")?($(this).closest(".well").animate({"background-color":"#f7f2c5"},300),$(this).addClass("fa-check-circle").removeClass("fa-check-circle-o")):($(this).closest(".well").animate({"background-color":"white"},300),$(this).addClass("fa-check-circle-o").removeClass("fa-check-circle"))})
